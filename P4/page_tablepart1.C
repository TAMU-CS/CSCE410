...
PageTable::PageTable()
{
   ...
   //page_directory[1023] = page_directory | 001
...
{
   unsigned long *page_directory = current_page_table->page_directory; // page_directory |1023|1023|0|
   unsigned long access_addr = read_cr2();
   unsigned long p1 = access_addr >> 22;
   unsigned long p2 = (access_addr & 0x003FFFFF) >> 12;

   if (!(page_directory[p1] & 1)) // |1023|1023|p1 + 00|
   {
      // allocate frame for page table
      unsigned int pt_frame_num = (unsigned int)kernel_mem_pool->get_frames(1); // process mem pool
      unsigned long *page_table = (unsigned long *)(pt_frame_num * PAGE_SIZE); // |1023|p1|0|
      ...
      for (unsigned int frame_num = 0; frame_num < ENTRIES_PER_PAGE; frame_num++)
      {
         ...
         page_table[frame_num] = 6;  // |1023|p1|frame_num + 00|
      }
